...
# ==============================
# RAG CHATBOT LOGIC (ĐÃ THÊM KEY CHO BUTTON & INPUT)
# ==============================
...
# ==============================
# GEMINI CHATBOT LOGIC (ĐÃ THÊM KEY CHO BUTTON & INPUT)
# ==============================
...
    econ_items = [
        ("Khu vực nhà nước", "structure_econ_state_vnd"),
        ("Doanh nghiệp ngoài quốc doanh", "structure_econ_nonstate_enterprises_vnd"),
...
    fig_e = make_bar(dfe, title="Thành phần kinh tế (bar nhỏ)")
    st.plotly_chart(fig_e, use_container_width=True)


# ---- Findings (GIỮ NGUYÊN) ----
with tab_find:
    st.header("Phân Tích Findings & Vi Phạm")
    st.markdown("---")
    
    c1, c2, c3 = st.columns([1,2,1])
    with c1:
        info_card("💸 **Tổng tiền ảnh hưởng**", format_vnd(f_df["quantified_amount"].sum()))
        info_card("👥 **Tổng hồ sơ ảnh hưởng**", f"{int(f_df['impacted_accounts'].sum()) if 'impacted_accounts' in f_df.columns and pd.notna(f_df['impacted_accounts'].sum()) else '—'}")
    with c2:
        # Biểu đồ Top 10 Legal Reference
        top_refs = f_df.groupby("legal_reference_chart")["quantified_amount"].sum().sort_values(ascending=False).head(10).reset_index()
        if not top_refs.empty:
            # Dùng altair cho biểu đồ Legal Reference
            chart_ref = alt.Chart(top_refs).mark_bar().encode(
                x=alt.X("quantified_amount", axis=alt.Axis(format='~s'), title="Tổng giá trị (VND)"),
                y=alt.Y("legal_reference_chart", sort="-x", title="Văn bản pháp lý"),
                color=alt.Color("legal_reference_chart", scale=alt.Scale(range=PALETTE), legend=None),
                tooltip=["legal_reference_chart", alt.Tooltip("quantified_amount", format=",.0f")]
            ).properties(
                title="Top 10 Legal Reference theo giá trị vi phạm"
            ).interactive()
            st.altair_chart(chart_ref, use_container_width=True)
        else:
            st.info("Không có dữ liệu findings nào phù hợp với bộ lọc Legal Reference.")
    with c3:
        # Biểu đồ Top 10 Root Cause
        top_roots = f_df.groupby("root_cause")["quantified_amount"].sum().sort_values(ascending=False).head(10).reset_index()
        top_roots = top_roots[top_roots["root_cause"].str.lower().ne("nan")].reset_index(drop=True)
        if not top_roots.empty:
            fig_roots = make_pie([(r["root_cause"], r["quantified_amount"]) for _, r in top_roots.iterrows()], title="Top 10 Root Cause theo giá trị vi phạm", height=400)
            st.plotly_chart(fig_roots, use_container_width=True)
        else:
            st.info("Không có dữ liệu Root Cause.")

    st.markdown("---")
    st.subheader("Chi tiết Findings (Đã Lọc)")
    if f_df.empty:
        st.info("Không có Findings nào phù hợp với bộ lọc Legal Reference hiện tại.")
    else:
        f_df["Quantified Amount"] = f_df["quantified_amount"].apply(format_vnd)
        f_df["Impacted Accounts"] = f_df["impacted_accounts"].apply(lambda x: int(x) if pd.notna(x) else "—")
        display_cols = [
            "category", "sub_category", "description", "legal_reference", "Quantified Amount", "Impacted Accounts", "root_cause", "recommendation"
        ]
        # Đổi tên cột để hiển thị tiếng Việt đẹp hơn
        display_df = f_df[display_cols].rename(columns={
            "category": "Category",
            "sub_category": "Sub Category",
            "description": "Mô tả",
            "legal_reference": "Văn bản pháp lý",
...
# ---- Actions (GIỮ NGUYÊN) ----
with tab_act:
    st.header("Chi Tiết Các Actions")
    st.markdown("---")
    if df_act.empty:
        st.info("Không có dữ liệu Actions.")
    else:
        display_cols_act = ["action_type", "legal_reference", "action_description", "evidence_of_completion"]
        display_df_act = df_act[display_cols_act].rename(columns={
            "action_type": "Loại Action",
            "legal_reference": "Văn bản pháp lý",
            "action_description": "Mô tả Action",
            "evidence_of_completion": "Bằng chứng hoàn thành"
        })
        st.dataframe(display_df_act, use_container_width=True, height=500)
