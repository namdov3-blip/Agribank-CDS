...
COL_MAP = {
    "documents": {
        ...
        "field_coverage": ["Field_coverage","field_coverage"],
    },
    "overalls": {
    ...
    if len(df_docs) == 0:
        ...
        for idx, row in df_docs.reset_index(drop=True).iterrows():
            ...
            with c1:
                ...
                info_card("Ngày ban hành KLTT", d.strftime("%d/%m/%Y") if pd.notna(d) else "—")
            with c2:
                d = row.get("issue_date", pd.NaT)
...
with tab_over:
    ...
    with k2:
        info_card("Phòng nghiệp vụ", f"{int(over_row.get('departments_at_hq_count', np.nan)) if pd.notna(over_row.get('departments_at_hq_count', np.nan)) else '—'}")
