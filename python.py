...
def call_gemini(messages: list):
    ...
    # KHẮC PHỤC LỖI #1: Lấy API key bằng đúng tên biến trong secrets.toml
    api_key = st.secrets["GEMINI_API_KEY"]
    
    # Lấy tên model (ví dụ: gemini-2.5-flash)
    model_name = _get_gemini_model_name() 

    # CHỈNH SỬA LOGIC KHẮC PHỤC LỖI 400 (Role Alternation)
    contents = []
    for m in messages:
        # API REST của Gemini sử dụng 'user' và 'model'
        role = "user" if m["role"] == "user" else "model"
        
        # Lấy nội dung, cắt an toàn
        content_text = m["content"][:20000]
    ...
    # KHẮC PHỤC LỖI #3: Truyền biến api_key vào URL (đã sửa lỗi NameError)
...
# GEMINI_MODEL = "gemini-2.5-pro"  # Hoặc "gemini-2.5-flash"
    ...
    for m in st.session_state.gemini_history:
        # Streamlit dùng 'assistant' cho Bot, Gemini API dùng 'model'
        display_role = "assistant" if m["role"] == "model" else "user"
